...
import { GacksDashboard } from './ui/GacksDashboard'
...
const NAME = '(?:gacks(?:\\s*p\\.?a\\.?)?|gacks\\s*pa|gacks|gax|gack|jarvis|jarvys|jervis|travis|jarviss|java\'s|jarv)'
...
export default function App() {
  ...
  // -- background preparation & intro handlers --------------------------------

  const startBackgroundServices = () => {
    if (backgroundServicesStarted.current) return
    backgroundServicesStarted.current = true
    ...
    watchCaptureScreen(async (req) => {
      return await captureScreen(req.reason)
    })
    ...
    watchUi((op, args) => {
      ...
      switch (op) {
        case 'patch':
          ...
          break
        case 'orbit':
          if (a.action === 'add') st.addOrbit(args)
          else if (a.action === 'remove') st.removeOrbit(String(a.id))
          else st.clearOrbits()
          break
        case 'effect':
          st.fireEffect(a.kind)
          break
        case 'reset':
          st.resetUi()
          break
        case 'screen':
          st.clearScreen(a.what ?? 'all')
          break
        default:
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      // H toggles between GACKS P.A Dashboard and legacy HUD mode
      if (e.key === 'h' && !e.repeat && !e.metaKey && !e.ctrlKey && !e.altKey) {
        e.preventDefault()
        setViewMode((m) => (m === 'dashboard' ? 'hud' : 'dashboard'))
        return
      }
  ...
  useEffect(() => {
    ...
    store.getState().clearPendingQuery()
    store.getState().setError(null)
