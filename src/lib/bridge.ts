...
let onCaptureScreen: ((req: { reason?: string }) => Promise<CaptureResult>) | null = null
export function watchCaptureScreen(fn: (req: { reason?: string }) => Promise<CaptureResult>) {
  onCaptureScreen = fn
}
  ...
  ws.addEventListener('message', (e: MessageEvent) => {
    ...
    if (msg.type === 'ready') {
      ...
      if (!onCaptureScreen) {
        ...
        onCaptureScreen({ reason: msg.reason ?? '' })
          .then(reply)
          .catch((err) => reply({ error: String(err?.message ?? err) }))
      }
